...
class RMSDMenu():
    ...
    def check_resolve_error(self, clear_only=False):
        if(not clear_only):
            if self._selected_mobile is None or self._selected_target is None:
    ...
    def change_complex_list(self, complex_list):
        ...
        if self._selected_mobile is None:
        ...
        if self._selected_target is not None and \
        ...
        for complex in complex_list:
            ...
            if self._selected_mobile is not None and btn.complex.index in [a.complex.index for a in self._selected_mobile]:
                btn.selected = True
            ...
            if self._selected_target is not None and btn.complex.index == self._selected_target.complex.index:
                btn.selected = True
    ...
    def build_menu(self):
        ...
        def run_button_pressed_callback(button):
            ...
            if self._selected_mobile is not None and self._selected_target is not None:
                self.show_loading_bar()
        ...
        def lock_button_pressed_callback(button):
            ...
            if self._selected_target is not None and len(self._selected_mobile) != 0:
        ...
        if len(self._selected_mobile) != 0 and self._selected_target is not None and self._selected_target.locked and mobile_locked:
            self.lock_img.add_new_image(file_path=os.path.join(os.path.dirname(__file__), LOCKICON))
        else:
        ...
        local_align_text = menu.root.find_node("Local Align txt", True).get_content()
